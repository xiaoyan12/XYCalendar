...
+ (NSInteger)firstWeekdayInThisMonth:(NSString *)dateStr{
    NSDate *date = [[[self class] setDataFormatter] dateFromString:dateStr];
...
+ (NSInteger)totaldaysInMonth:(NSDate *)date {
    ...
    return [[NSString stringWithFormat:@"%lu",daysInLastMonth.length] integerValue];
}
...
+ (NSString *)getCurrentTime{
    ...
    return [[[self class] setDataFormatter] stringFromDate:date];
}
...
{
    NSDate *nowDate = [[[self class] setDataFormatter] dateFromString:currentTime];
    NSDate *theDate;
    if(day!=0){
    ...
    return [[[self class] setDataFormatter] stringFromDate:theDate];
}
...
+ (NSInteger)getWeekDayFromDate:(NSString *)dateString{
    NSDate *date = [[[self class] setDataFormatter] dateFromString:dateString];
...
+ (int)getCurrentMonthDayNum:(NSString *)deteStr{
    NSDate *date = [[self setDataFormatter] dateFromString:deteStr];
    return [[NSString stringWithFormat:@"%ld",[self totaldaysInMonth:date]] intValue];
}
...
+ (NSString *)getNextYearWithMonth:(int)month startYear:(NSInteger)year index:(int)index{
    ...
    NSString *dateStr = [[[self class] setDataFormatter] stringFromDate:nextDate];
    return [NSString stringWithFormat:@"%ld",[[self class] year:dateStr]];
}
